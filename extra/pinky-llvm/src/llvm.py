...
class LLVMGenerator:
    def generate(self, node, module):
        ...
        if isinstance(node, String):
            compile_error(f'Strings are not implemented in the current LLVM IR generator.', node.line)
        ...
        if isinstance(node, BinOp):
            ...
            if node.op.token_type == TOK_PLUS:
                ...
                elif lefttype == TYPE_NUMBER and righttype == TYPE_NUMBER:
            ...
            if node.op.token_type == TOK_MOD:
                ...
                if lefttype == TYPE_NUMBER and righttype == TYPE_NUMBER:
                    ...
                    return (TYPE_BOOL, module.builder.fcmp_signed('==', leftval, rightval))
                else:
                    compile_error(f'Unsupported operator {node.op.lexeme!r} between {lefttype} and {righttype}.', node.op.line)
        ...
        if isinstance(node, UnOp):
            ...
            if node.op.token_type == TOK_MINUS:
            ...
            elif node.op.token_type == TOK_PLUS:
                if operandtype == TYPE_NUMBER:
                ...
                if operandtype == TYPE_BOOL:
                    return (TYPE_BOOL, module.builder.not_(operandval))
                else:
                    compile_error(f'Unsupported operator {node.op.lexeme!r} with {operandtype}.', node.op.line)
        ...
        if isinstance(node, LogicalOp):
            ...
            if node.op.token_type == TOK_OR:
                return (TYPE_BOOL, module.builder.or_(leftval, rightval))
            elif node.op.token_type == TOK_AND:
                return (TYPE_BOOL, module.builder.and_(leftval, rightval))
        ...
        if isinstance(node, PrintStmt):
            ...
            if exprtype == TYPE_NUMBER:
                module.builder.call(module.print_f64, [exprval])
            elif exprtype == TYPE_BOOL:
                module.builder.call(module.print_i1, [exprval])
        ...
        if isinstance(node, WhileStmt):
            ...
            # Body
            self.generate(node.stmts, module)
            module.builder.branch(test_label)
            # Exit
            module.builder.position_at_end(exit_label)
    ...
    def generate_main(self, node):
        ...
        module.builder.ret(ir.Constant(i32, 0))
        return module
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('Usage: python llvm.py <filename>')
    filename = sys.argv[1]
